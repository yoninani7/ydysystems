-- ============================================================
--  YDY HRM Enterprise — Full Normalized MySQL Schema
--  Version : 1.0.0
...
--    • Surrogate INT PKs (AUTO_INCREMENT) on every table
--    • Natural / business keys kept as UNIQUE constraints
--    • FK references always indexed (MySQL does NOT auto-index FKs)
--    • Composite indexes on every high-frequency JOIN / filter pattern
--    • ENUM for small, stable value sets  →  faster than VARCHAR lookups
--    • DECIMAL(15,2) for all money  →  no floating-point rounding errors
--    • Soft-delete pattern via `deleted_at` (no permanent data loss)
--    • `created_at` / `updated_at` auto-maintained by MySQL triggers
-- ============================================================
...
CREATE TABLE companies (
  ...
  legal_name          VARCHAR(200)    NOT NULL COMMENT 'Official registered legal name',
  trading_name        VARCHAR(200)        NULL COMMENT 'Brand / trading name',
  ...
  registration_no     VARCHAR(80)     NOT NULL COMMENT 'Trade/ministry registration number',
  tin                 VARCHAR(30)         NULL COMMENT 'Tax Identification Number',
  ...
  -- Policies
  work_week_desc      VARCHAR(100)        NULL COMMENT 'e.g. Mon–Fri 40hrs + Sat half-day',
  probation_days      SMALLINT UNSIGNED   NULL DEFAULT 90,
  retirement_age      TINYINT UNSIGNED    NULL DEFAULT 60,

  -- Treasury
  main_bank           VARCHAR(100)        NULL,
  bank_account        VARCHAR(50)         NULL,
  base_currency       CHAR(3)             NULL DEFAULT 'ETB' COMMENT 'ISO 4217 currency code',
  fiscal_year_start   VARCHAR(50)         NULL COMMENT 'e.g. Hamle 01 (July 08)',

  -- Digital
  website             VARCHAR(200)        NULL,
  support_email       VARCHAR(150)        NULL,
  corporate_phone     VARCHAR(30)         NULL,
...
CREATE TABLE branch_offices (
  id              INT UNSIGNED    NOT NULL AUTO_INCREMENT,
  company_id      INT UNSIGNED    NOT NULL COMMENT 'FK → companies',
